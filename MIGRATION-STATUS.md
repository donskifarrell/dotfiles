...
## Resume here
Next action: abhaile is DONE (on Den + sops-nix + systemd-boot across a reboot). Remaining, lower-risk:
Phase 5b flake update (isolated), Phase 6 fresh eachtrach (nixos-anywhere + deploy-rs), Phase 7 remove clan
+ doc cleanup, Phase 8 optional agenix. Pick up at whichever the user wants next.
Blocked on: nothing. (Phase 6 needs a new eachtrach VM to provision; user-driven.)

## Day-2 deploy / rollback (abhaile, verified working)
- Build+switch:  sudo nixos-rebuild switch --flake .#abhaile
  (bootloader is systemd-boot now; --install-bootloader is NOT needed again.)
- Rollback:      pick the previous generation in the systemd-boot menu at boot, OR
                 sudo nixos-rebuild switch --rollback
- Host age identity: /etc/ssh/ssh_host_ed25519_key (ssh-to-age => age1ggl…). Editor key: df ~/.config/sops/age/keys.txt.
- Edit secrets:  sops secrets/shared.yaml | sops secrets/abhaile.yaml  (re-key new hosts: add recipient to
  .sops.yaml then `sops updatekeys secrets/<f>.yaml`).
...
- 2026-06-28 · phase 5 · CUTOVER DONE. Host-key installed at /etc/ssh/ssh_host_ed25519_key first.
  grub->systemd-boot transition gotchas: (1) plain `nixos-rebuild switch` took the systemd-boot UPDATE
  path and crashed (bootctl found no systemd-boot) -> MUST use `--install-bootloader` for the first switch.
  (2) ESP (500M) was 100% full: grub's /boot/kernels (282M) + /boot/grub (16M) + theme files overflowed
  systemd-boot's generation copies -> removed grub leftovers (freed ~300M), re-ran, completed. Activation
  ran sops OK. Rebooted: gen 92 den-built, systemd-boot, 0 failed units, host id age1ggl…, df home secrets
