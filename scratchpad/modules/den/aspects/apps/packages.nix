# Ported from modules/home/packages.nix. A package catalog exposing
# `my.packages.<name>.enable` toggles, plus `my.packages.enableAll`. A role or
# host's home-manager block flips the toggles it wants (see roles/desktop.nix).
{
  den.aspects.apps.packages.homeManager =
    {
      lib,
      pkgs,
      config,
