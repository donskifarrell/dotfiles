# Ported from modules/home/fish.nix. The legacy module took a `flakeHostname`
# specialArg for the `os-switch` abbr; that is dropped here — `nh` detects the
# current host, so `nh os switch ~/.dotfiles` is enough.
{
  den.aspects.apps.shell.fish.homeManager =
    { pkgs, ... }:
    {
      programs.fish = {
        enable = true;
