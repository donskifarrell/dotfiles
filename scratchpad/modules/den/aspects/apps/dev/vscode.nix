...
# nix-vscode-extensions marketplace overlay, which the legacy repo applied
# globally in nix-config.nix. Here the overlay is scoped to this aspect's
# `nixos` block (applied to the system nixpkgs, which home-manager shares via
# useGlobalPkgs) so it only lands on hosts that include vscode.
{ inputs, ... }:
{
